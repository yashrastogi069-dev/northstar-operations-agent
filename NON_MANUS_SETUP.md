...
For TiDB Cloud, keep the complete connection string generated by TiDB, including the `gateway.tidbcloud.com` host, port `4000`, database name, username, and URL-encoded password. Northstar now parses that URL and enables TLS certificate validation by default. Add `DATABASE_SSL=true` and `DATABASE_SSL_REJECT_UNAUTHORIZED=true` explicitly in Render, and optionally set `DATABASE_CONNECTION_LIMIT=5`. Only a local development database should use `DATABASE_SSL=false`; never use that production override to bypass a TiDB certificate or network problem.

### Apply the knowledgeChunks embedding migration

The embedding column is additive and preserves all existing documents and chunks. From a trusted environment with the production `DATABASE_URL` available, run:
...
Do not run `db:push` against production for this repair because it regenerates schema changes. Verify the result in TiDB with:

```sql
SHOW COLUMNS FROM knowledgeChunks LIKE 'embedding';
```

The command should return one nullable JSON column named `embedding`. If a run fails with `Unknown column 'knowledgeChunks.embedding'`, apply migration `drizzle/0006_chief_meteorite.sql` once, restart the Render service, and retry the run. The corrected Northstar UI will identify this as schema drift rather than a TLS failure.
