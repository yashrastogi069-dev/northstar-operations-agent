...
export const agentRouter = router({
  ...
  runs: router({
    ...
    get: protectedProcedure.input(z.object({ runId: z.number().int().positive() })).query(async ({ ctx, input }) => {
      ...
      return { run, snapshots: await listAgentStateSnapshots(run.id), toolCalls: await listAgentToolCalls(run.id), artifacts: await listAgentArtifacts(run.id) };
    }),
    start: protectedProcedure.input(runInput).mutation(async ({ ctx, input }) => {
      const decision = classifyRequest(input.request, Boolean(input.dataText?.trim()));
      const threadId = `northstar_${nanoid(20)}`;
      const runId = await createAgentRun({ threadId, title: input.title ?? input.request.slice(0, 96), request: input.request, taskType: decision.taskType, riskTier: decision.riskTier, ownerUserId: ctx.user.id });
      await addAuditEvent({ actorUserId: ctx.user.id, eventType: "agent.run_started", entityType: "agent_run", entityId: runId, severity: decision.riskTier === "blocked" ? "warning" : "info", details: { threadId, taskType: decision.taskType, riskTier: decision.riskTier, dataProvided: Boolean(input.dataText?.trim()) } });
      try {
        ...
        await addAuditEvent({ actorUserId: ctx.user.id, eventType: `agent.run_${state.status}`, entityType: "agent_run", entityId: runId, severity: state.status === "blocked" || state.status === "failed" ? "warning" : "info", details: { threadId, toolNames: state.toolResults.map(tool => tool.toolName), artifactId, approvalId, retryCount: state.retryCount } });
        ...
        await addAuditEvent({ actorUserId: ctx.user.id, eventType: "agent.run_failed", entityType: "agent_run", entityId: runId, severity: "high", details: { errorMessage } });
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Northstar could not complete this run. The failed run is available for review." });
      }
    }),
  }),
  approvals: router({
