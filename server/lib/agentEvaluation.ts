...
export type AgentEvaluationInput = { scenarioName: string; request: string; expectedPolicy: EvaluationExpectation; expectedTools?: AgentToolName[]; hasData?: boolean };
...
export function evaluateAgentScenario(input: AgentEvaluationInput) {
  ...
  const plan = decision.allowed ? createDeterministicPlan(input.request, decision, Boolean(input.hasData)) : null;
